...
//!
  ...
  const QIcon runIcon =
      QIcon::fromTheme("edit-paste", QIcon(":/images/paste.png"));
  QAction *runAction = new QAction(runIcon, tr("&Run Model Generator"), this);
  runAction->setShortcuts(QKeySequence::Refresh);
  runAction->setStatusTip(tr("Run The current script"));
  editMenu->addAction(runAction);
  editToolBar->addAction(runAction);
  connect(runAction, &QAction::triggered,
          [this]() { emit runScript(curFile); });
  const QIcon runLineIcon =
      QIcon::fromTheme("edit-paste", QIcon(":/images/paste.png"));
  QAction *runLineAction = new QAction(runIcon, tr("&Run Line"), this);
  runLineAction->setShortcuts(QKeySequence::Refresh);
  runLineAction->setStatusTip(tr("Run current line in terminal"));
  editMenu->addAction(runLineAction);
  editToolBar->addAction(runLineAction);
  connect(runLineAction, &QAction::triggered, [this]() {
    auto line = textEdit->textCursor().block().text().trimmed();
    emit runLine(line);
  });
...
// https://stackoverflow.com/a/34238144/4372670
bool MainWindow::event(QEvent *event) {
  qDebug() << event->type();
  if (event->type() == QEvent::Show || event->type() == QEvent::ShowToParent) {
    // qDebug() << "new text editor:";
    emit focusChanged(textEdit);
  }
