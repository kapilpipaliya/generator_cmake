...
ControllerEntry::ControllerEntry(const std::vector<std::string> &controllers,
    ...
    if (eventadd) {
      ...
      includeline += ".h\"";
      {
        if (pystring::find(todoactorcontent, includeline) == -1) {
          std::smatch match;
          todoactorcontent =
              pystring::replace(todoactorcontent, "// auto_include_above",
                                includeline + "\n// auto_include_above");
        }
      }
      std::string eventline;
      eventline +=
          "    if (event1 == " + pystring::lower(enumname) + ") {" + "\n";
      eventline += "      handleService<timeservice::" + className +
                   ...
                   "\n    }";
      {
        if (pystring::find(todoactorcontent, eventline) == -1) {
          std::smatch match;
          todoactorcontent =
              pystring::replace(todoactorcontent, "    // auto_event_above",
                                eventline + "\n    // auto_event_above");
        }

        {
          if (pystring::find(routefilecontent, pystring::lower(enumname)) ==
              -1) {
            std::smatch match;
      ...
      {
        std::string c1 = "AUX_SOURCE_DIRECTORY(./" +
                         pystring::lower(std::regex_replace(className, regex,
                                                            std::string("/"))) +
                         " SRC)";
        if (pystring::find(cmakecontent, c1) == -1) {
          std::smatch match;
          cmakecontent = pystring::replace(cmakecontent, "#auto_source_above",
                                           c1 + "\n#auto_source_above");
        }
      }
    }
    if (protoadd) {
      std::string c2 = "AUX_SOURCE_DIRECTORY(./" +
                       pystring::lower(std::regex_replace(className, regex,
                                                          std::string("/"))) +
                       "/proto SRC)";
