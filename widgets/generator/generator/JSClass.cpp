...
JSClass::JSClass(const std::vector<std::string> &controllers,
...
std::string JSClass::generate(std::string namespacereq, bool commentreq,
...
filename:auth/kapil/Login.svelte
...

    {
      ...
      if (!routeFileName.empty()) {
        std::string routeFileFullPath =
            "/home/kapili3/k/svelte/sapper/time_management/src/routes/" +
            routeFileName;
        ...
        createRouteFiles(oRouteFile, className, routeFileFullPath,
                         content_route, count, namespacereq, commentreq,
                         namespaceres, commentres);
      }
    }
...
void JSClass::createRouteFiles(std::ofstream &file,
  ...
  std::string fileName_ =
      std::regex_replace(className, std::regex("::"), std::string("_"));
