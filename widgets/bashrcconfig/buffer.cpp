...
Buffer::Buffer() : QObject(nullptr) {
  DEBUG_ENTER(Buffer::Buffer);
  DEBUG_EXIT(Buffer::Buffer);
}

Buffer::Buffer(const QString source) : QObject(nullptr) {
  DEBUG_ENTER(Buffer::Buffer);
  setSource(source);
  DEBUG_EXIT(Buffer::Buffer);
}

Buffer::Buffer(Buffer &copy) : QObject(copy.parent()) {
  DEBUG_ENTER(Buffer::Buffer);
  *this = copy;
  DEBUG_EXIT(Buffer::Buffer);
}

Buffer::Buffer(Buffer &&move) : QObject(move.parent()) {
  DEBUG_ENTER(Buffer::Buffer);
  *this = move;
  DEBUG_EXIT(Buffer::Buffer);
}
