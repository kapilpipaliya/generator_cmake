...
Searcher::Searcher(Searcher &copy) : QObject(copy.parent()) {
  DEBUG_ENTER(Searcher::Searcher);
  *this = copy;
  DEBUG_EXIT(Searcher::Searcher);
}

Searcher::Searcher(Searcher &&move) : QObject(move.parent()) {
  DEBUG_ENTER(Searcher::Searcher);
  *this = move;
  DEBUG_ENTER(Searcher::Searcher);
}
...
int Searcher::templateSearch(Type search, int from) {
  ...
  Buffer::State
      *q1 = nullptr,
      *q2 = nullptr,
      *q3 = nullptr;  // do not have to worry about every referencing a nullptr
