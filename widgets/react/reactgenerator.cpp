...
void ReactGenerator::exportgenerator(QHBoxLayout *hbox)
{
  ...
  auto generate = [typeselect, name, exporttval, defaultval, checkbox, resultText]() {
    ...
    if (exporttval->text().isEmpty())
    {
      ...
      {
        if (checkbox->isChecked()) { r += " = undefined;"; }
        else
        {
          r += ";";
        }
...
void ReactGenerator::reactivestatgenerator(QHBoxLayout *hbox)
{
  auto g = new QGroupBox(this);
  ...
  auto generate = [name, simplevalue, functionvalue, resultText]() {
    ...
    QString r;
    r += "$: ";
    if (!simplevalue->text().isEmpty()) { r += nameText + " = " + simplevalue->text() + ";"; }
    else
    {
...
void ReactGenerator::elementdirectivesgenerator(QHBoxLayout *hbox)
{
  auto g = new QGroupBox(this);
  ...
  h->addWidget(resultLabel);
  h->addWidget(resultText);
  auto generate = [directiveselect, eventname, modifiersselect, handlertval, resultText]() {
    ...
    for (auto &item : modifiersselect->selectedItems()) { v.push_back(item->text().toStdString()); }
    auto j = pystring::join("|", v);
    if (!j.empty()) { r += "|"; }
    r += QString::fromStdString(j);
    if (!handlertval->toPlainText().isEmpty())
    {
      if (!isSame || directiveselect->currentText() == "on:") { r += "='{" + handlertval->toPlainText() + "}'"; }
    }
    resultText->setText(r);
  };
