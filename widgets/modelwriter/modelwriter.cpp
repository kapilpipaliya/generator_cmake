...
ModelWriter::ModelWriter(QWidget *parent)
  ...
  connect(addbutton, &QToolButton::clicked,
          [this, verticalLayout1, key, value]() {
...
void ModelWriter::setGoSaveTemplate(QString key, QString default_value,
                                    QToolButton *go, QToolButton *save,
                                    QToolButton *delete_button, QGroupBox *box,
                                    QCodeEditor *templ, QCodeEditor *result,
...
void ModelWriter::setTemplResult(QCodeEditor *templ, QCodeEditor *result) {
  ...
  QString new_result;
  if (templ) {
    new_result = QString(templ->toPlainText())
                     .arg(toCamelCase(text))
                     .arg(text.toLower())
                     .arg(columns);
  }
  if (result) {
    result->setPlainText(new_result);
  }
}
...
void ModelWriter::set(QString key, QString value) {
  Orm::drogonModelSaveSetting(key.toStdString(), value.toStdString());
}

QString ModelWriter::get(QString key) {
  return QString::fromStdString(Orm::drogonModelGetValue(key.toStdString()));
}
...
QGroupBox *ModelWriter::groupboxCreate(QString key, QString value) {
  ...
  setGoSaveTemplate(key, value, goButton, saveButton, deleteButton, groupbox,
                    templ, result, slot);
