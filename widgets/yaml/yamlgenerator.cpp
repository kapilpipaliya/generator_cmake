...
namespace widgets
{
YamlGenerator::YamlGenerator(QWidget *parent) : QWidget(parent), ui(new Ui::YamlGenerator)
{
  ui->setupUi(this);
  ...
  buildset();
  convertofrom();
  connect(ui->toolButton_inter_clear, &QToolButton::clicked, [this]() { ui->textEdit_inter->setText(""); });
}
...
void YamlGenerator::setGoSaveTemplate(QString key, QString default_value, QToolButton *go, QToolButton *save, QPlainTextEdit *templ, std::function<void()> slot)
{
...
QString YamlGenerator::get(QString key) { return ""; /*QString::fromStdString(Orm::yamlGetValue(key.toStdString()));*/ }
void YamlGenerator::on_pushButton_clicked() { QDesktopServices::openUrl(QUrl("https://github.com/jbeder/yaml-cpp/wiki/Tutorial", QUrl::TolerantMode)); }
}  // namespace widgets
