...
ReminderWidget::ReminderWidget(QWidget *parent) : QWidget(parent) {
  ...
  taskaddhorizontalLayout->setObjectName(
      QString::fromUtf8("taskaddhorizontalLayout"));
  ...
  auto slot = [this]() {
    ...
    QString lasttask;
    if (rowcount > 0) {
      lasttask = tableWidget_tasks->item(rowcount - 1, 1)
                     ->data(Qt::DisplayRole)
                     .toString();
    }
  ...
  connect(lineEdit_task, &QLineEdit::returnPressed,
          [this]() { addTask(lineEdit_task->text()); });
  connect(toolButton_add, &QToolButton::clicked,
          [this]() { addTask(lineEdit_task->text()); });
}
...
void ReminderWidget::addTask(QString task) {
  ...
  if (rowcount > 0) {
    lasttask = tableWidget_tasks->item(rowcount - 1, 1)
                   ...
                   .toString();
    lasttaskid =
        tableWidget_tasks->item(rowcount - 1, 0)->data(Qt::DisplayRole).toInt();
    finished = tableWidget_tasks->item(rowcount - 1, 3)
                   ->data(Qt::DisplayRole)
                   .toString();
  }
  if (task != lasttask) {
    ...
    if (rowcount > 0 && finished.isEmpty()) {
      ...
      if (parentWidget) {
        QToolButton *stop_button =
            parentWidget->findChild<QToolButton *>("stop");
...
void ReminderWidget::addTaskToTable(ReminderSetting r) {
  ...
  if (finished.isEmpty()) {
    ...
    connect(stop_button, &QToolButton::clicked,
            [id, stop_button, finishedItem, durationItem]() {
