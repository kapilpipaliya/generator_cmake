...
WsTest::WsTest(QWidget *parent) : QWidget(parent), ui(new Ui::WsTest) {
  ...
  connect(addbutton, &QToolButton::clicked,
          [this, verticalLayout1, spinboxEdit, event_nameEdit, argEdit,
           payloadEdit, valueEdit]() {
            ...
            auto w = Orm::wstestSaveSetting(
                number, event_name.toStdString(), arg.toStdString(),
                payload.toStdString(), value.toStdString());
            auto groupbox =
                addTest(w.id, number, event_name, arg, payload, value);
            verticalLayout1->addWidget(groupbox);
...
QGroupBox *WsTest::addTest(int id, int number, QString event_name, QString arg,
...
void WsTest::setGoSaveTemplate(int id, QToolButton *go, QToolButton *save,
  ...
  connect(save, &QToolButton::clicked,
          [id, spinbox, event_nameEdit, argEdit, payload, templ]() {
            Orm::wstestUpdateSetting(id, spinbox->value(),
                                     event_nameEdit->text().toStdString(),
                                     argEdit->toPlainText().toStdString(),
                                     payload->toPlainText().toStdString(),
                                     templ->toPlainText().toStdString());
          });
