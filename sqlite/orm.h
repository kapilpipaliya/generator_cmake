...
inline auto initStorage(const std::string &path) {
  ...
  return make_storage(
      ...
      make_table(
          "drogon_model_settings",
          make_column("id", &DrogonModelSetting::id, autoincrement(),
                      primary_key()),
          make_column("key", &DrogonModelSetting::key),
          make_column("template_string", &DrogonModelSetting::template_string)),
      make_table(
          "sql_compare_settings",
          make_column("id", &SQLCompareSetting::id, autoincrement(),
                      primary_key()),
      ...
      make_table("reminder_settings",
                 make_column("id", &ReminderSetting::id, autoincrement(),
                             primary_key()),
      ...
      make_table("wstest_settings",
                 make_column("id", &WsTestSettingSetting::id, autoincrement(),
                             primary_key()),
...
class Orm {
 public:
  ...
  static WsTestSettingSetting wstestSaveSetting(int number,
                                                std::string event_name,
                                                std::string arg,
                                                std::string payload,
                                                std::string result);
  static WsTestSettingSetting wstestUpdateSetting(int id, int number,
                                                  std::string event_name,
                                                  std::string arg,
                                                  std::string payload,
                                                  std::string result);
